...
- **Every Python file must be covered by `make typecheck`.** When adding a new Python service, package, or standalone script, immediately add it to the mypy invocation in the Makefile. No Python file may exist outside mypy's scope. Files added inside an existing whole-directory target (e.g. a new module in `cli/`, a new file in `services/relay_core/`) are covered automatically — no Makefile change needed.
